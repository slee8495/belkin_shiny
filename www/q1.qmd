...
I will go ahead and load the neccessary packages to perform the analysis
...
In order to do that, let's examine the data first. 
...
Looking at the dataset, I have to define which variable should be the response variable, and determine the best independent variables. 


before I deep dive into the statistical modeling, I'd like to see if the data is usable for the analysis. 

Let's see how the data is structured and if there are any missing values.
...
##### Now, given that there is no missing data, and the data is well cleaned and good to use. I will go ahead compute the metrics that I would propose to monitor over time to define the success of the team’s efforts in improving the revenue.
...
I will now try to refine the model by removing the non-significant predictors and re-evaluating the model fit.
...
### Insights from the Refined Model (Now, the overall model becomes significantly improved in terms of the p-value)
